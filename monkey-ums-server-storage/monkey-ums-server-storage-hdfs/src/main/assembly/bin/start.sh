...
JAVA_OPTS=" -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Dconfig-factory=com.monkeyframework.config.PropertyConfigurerFactory "
JAVA_DEBUG_OPTS=""
if [ "$2" = "debug" ]; then
...
nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.monkeyframework.flume.node.Application env=$ENV --conf-file conf/event.conf --name event > $STDOUT_FILE 2>&1 &
